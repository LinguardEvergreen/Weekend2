#include <stdio.h>      //Ai fini dell'esecuzione dell'eseguibile, è consigliato mettere la libreria <stdlib.h> per inserire la funzione system("pause")
...

{
        ...
        scanf ("%d", &scelta);  //Inserimento della scelta data dal menù errata poiché la variabile è stata dichiarata come carattere mentre nello scanf viene inserita come un intero
...
return 0;       //Restituisce valore 0 alla funzione main e termina il programma. Per pura scelta grafica e per una lettura migliore, è consigliabile indentarlo
...
void moltiplica ()      //Questa funzione moltiplica () serve a moltiplicare 2 numeri dati in input.
{
        ...
        scanf ("%f", &a);       //Inserimento errato del primo numero poiché %f fa riferimento alle variabili di tipo float
        scanf ("%d", &b);       //Inserimento errato del secondo numero poiché %d fa riferimento alle veriabili di tipo int
...
void dividi ()  //Questa funzione dividi() serve a dividere 2 numeri dati in input
{
        ...
        scanf ("%d", &a);
        printf ("Inserisci il denumeratore:");          //Stampa errata poiché si scrive denominatore
        scanf ("%d", &b);

        int divisione = a % b;  //Dichiarazione ed assegnazione completamente sbagliata poiché dichiara la variabile divisione come un intero, senza considerare una possibile soluzione con la virgola
                                //e soprattutto l'assegnazione è sbagliata poiché non fa la divisione la il modulo tra a e b, il quale serve a calcolare il RESTO di una funzione. 
